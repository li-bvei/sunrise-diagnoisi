...
- 工资、社保与到手金额：基本给以万日元输入，另可填通勤手当（月15万円以内非课税，但计入社保基数）、课税手当、其他非课税手当（出差费等实费性质，不课税、不计社保），明细按给与明细书列出各手当行；同时给出员工到手、到手比例、公司实际雇佣成本、健康保险/厚生年金等级与临界区间；住民税按标准控除自动概算，可手动调整
- 役员报酬模拟：单一计算器，只需以万日元输入役员年工资，自动折算月工资，并给出月度/年度的社保与税金支付明细（给与明细书样式的支给/控除表）；不涉及公司利润、法人税或多方案对比。
  月額・年額两张表各有独立的「下载 PDF」和「下载 CSV」：PDF 在浏览器端生成（`html2canvas` + `jsPDF`，仅在点击时懒加载，不进主包），以图像方式排版所以中日文无需内嵌字体，代价是 PDF 里的文字不可选中/搜索；CSV 为 UTF-8 BOM，可直接用 Excel 打开
- 高度人才报告预览与租房费用清单报告均为在线预览，不含打印/另存为 PDF 功能，相关 CSS 与按钮已移除
- 页面本身按 Cmd/Ctrl+P 打印时使用一份精简的打印样式（`index.css` 末尾 `@media print`）：不打印顶栏/手机菜单/页脚/下载按钮，面板不会被拆到两页，上下页边距 18mm 给浏览器自带的页眉页脚留位。工资页和役员报酬页打印为一页 A4：输入表单不打印，改为标题 + 1～2 行条件摘要（`PrintHeader` 组件）。这两页及其 PDF 不带说明段落和免责声明，仅在项目名里保留“（概算）”
...
- 完整报告在线预览包含总分进度图、分类条形图、70/80 分状态、精简积分明细、共享建议、院校判断与免责声明；不再显示官方依据、证明项目和人工确认区块；当前版本不提供打印/导出 PDF
- 结果页与报告的积分明细统一只显示项目、分数和状态
- 日期输入值统一为 `YYYY/MM/DD`，日历月份强制使用 `1月` 至 `12月` 的阿拉伯数字；年收入页面按“万日元/万円”输入，计算层仍保存日元整数
- 租房初期费用诊断：13 项预设费用（敷金、礼金、保证金、仲介手续费、保证公司利用料、火灾保险、钥匙更换费、当月/次月房租、当月/次月管理费、清扫费、契约事务手续费）+ 8 项预设材料清单，均为勾选后手动输入金额，支持自定义追加项目，当月房租提供按天折算的参考值供一键填入，生成类似高度人才积分诊断的可查看报告
...
- 工资与役员报酬工具的所得税/住民税仍为标准控除概算：未覆盖配偶/扶养控除、税额控除与自治体超过课税，正式方案仍需人工复核
...
报告以弹窗形式在线预览（两类图表、三列积分明细、建议、院校判断和免责声明），当前版本不提供打印/导出 PDF 入口。
