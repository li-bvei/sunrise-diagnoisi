...
> 当前项目的统一交接入口是 [`docs/PROJECT_HANDOFF.md`](docs/PROJECT_HANDOFF.md)，修改建议位于 [`docs/CHANGE_RECOMMENDATIONS.md`](docs/CHANGE_RECOMMENDATIONS.md)。本文件保留较早阶段的业务约束和设计决策；如果其中的工具数量、已完成功能或实现状态与当前源码不一致，以当前源码、测试和交接文档为准。

## 最终目标

- 形成面向客户的专业诊断前台，并逐步接入案件、材料和跟进能力；当前工具数量以 `frontend/src/data/tools.ts` 和 `docs/PROJECT_HANDOFF.md` 为准。
- 姓名和电话号码不是所有工具的统一必填项，是否采集必须由具体业务、隐私政策和用户同意共同决定。
- 出生日期不是全局必填项，只在高度人才、永住等确实需要时显示。工资与役员报酬工具只用年龄，不收集出生日期。
...
> 以下顺序是早期产品计划，不是当前完成状态。当前工作项和优先级以 `docs/CHANGE_RECOMMENDATIONS.md` 为准。
...
> 当前代码已经采用简化的预计总分/逐项明细模型；本节保留早期设计背景，不能当作当前 UI 或结果结构的实现承诺。
