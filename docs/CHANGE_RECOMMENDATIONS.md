...
下面正文写于 2026-09-06，此后代码有不少变化。**读正文前先看这张表**（“已完成/部分/未做/已不适用”均按当前 `main` 核对）：
...
| 2. 测试合并为单一命令 | **已完成** | `npm test` = `vue-tsc -b && node --test tests/*.test.mjs`，当前 48 项；未拆 `test:regression` 等子脚本；无 CI |
...
| 5. 宅建记录 | **基本已具备** | 导入的 CSV 会保存文件名和导入时间，界面显示“已导入 N 条”，并有清除入口；存储版本迁移和历史上限 20 已有 |
| 6. 规则元数据、参数分类 | **未做** | 没有 `RuleMetadata`；官方常量/简化估算/用户输入未区分 |
| 6. 负利润被截成 0 | **已不适用** | 役员报酬的公司利润、法人税整块已按业主要求删除（2026-09-20） |
| 6. 住民税由用户输入 | **已解决** | 现按年收自动概算，可手动改写，界面标“概算” |
| 7. 正式法律页 | **未做** | 仍是占位；高度人才/永住仍在诊断前要姓名（必填）、电话（选填） |
| 8. 前端体积 | **部分** | PDF 相关库懒加载、不进主包；主 JS 仍约 1,045 KB、CSS 约 419 KB，题库数据在 ~379 KB 的 chunk 里 |
| 8. 日期组件 `MutationObserver` | **未做** | `App.vue` 里仍在监听整个 `body` |
| 8. 标题单一来源 | **未做** | `document.title` 仍由 `PublicLayout.vue` 与 `router/index.ts` 两处设置 |
| 9. 无障碍验收 | **未系统做** | 无逐页键盘/读屏验收记录 |
| 4.“保存/打印”结果动作 | **按决定不做** | 高度人才/租房报告的打印与另存 PDF 已按业主要求删除；仅役员报酬页有 PDF/CSV 下载，页面级打印用精简打印样式；工资/役员报酬页打印为一页 |
| 10–12 ERM / 后端 / Sprint 3–4 | **未开始** | 仍无后端；咨询留资只有方案，见 `PROJECT_HANDOFF.md` 附录 |

正文里提到“11 个工具”“年金”“成本超出利润”等，均是写作时的状态，现已不成立（现为 7 个工具，年金已下线）。
...
- ~~将宅建题库、考点和薄弱分析数据继续做路由级或数据级懒加载~~（已解决：题库和考点改为从 API 读取，不再打进前端包）；
